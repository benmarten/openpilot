  ...
  def update_events(self, CS):
    ...
    if CS.canValid:
      ...
      if main_button_pressed and always_on_lateral and aol_on_main and not self.enabled:
        self.events.add(EventName.preEnableStandstill)  # Activates preEnabled state for lateral-only

      # Disable on rising edge of accelerator or brake. Also disable on brake when speed > 0
      # ENHANCED: Feature 3 - AOL: Keep lateral active when pedals pressed
      if (CS.gasPressed and not self.CS_prev.gasPressed and self.disengage_on_accelerator) or \
        (CS.brakePressed and (not self.CS_prev.brakePressed or not CS.standstill)) or \
        (CS.regenBraking and (not self.CS_prev.regenBraking or not CS.standstill)):
        if not always_on_lateral:
          # Stock behavior: pedalPressed event disengages both lateral and longitudinal
          self.events.add(EventName.pedalPressed)
        else:
          # AOL enabled: Don't add pedalPressed event to keep lateral active
          # Longitudinal control is naturally overridden by pedal input at the car interface level
          # The CarController sees pedal input and backs off on throttle/brake commands
          # while lateral (steering) commands continue
          pass
